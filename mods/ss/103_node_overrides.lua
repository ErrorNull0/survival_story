...
local mt_dig_node = core.dig_node
local mt_remove_node = core.remove_node
local mt_add_entity = core.add_entity
local mt_get_pointed_thing_position = core.get_pointed_thing_position
...
core.override_item("ss:stone", {
	...
	on_flood = function(pos, oldnode, newnode)
		core.add_item(pos, ItemStack("ss:stone"))
	end,
...
core.override_item("ss:stick", {
	...
	floodable = true,
	on_flood = function(pos, oldnode, newnode)
		core.add_item(pos, ItemStack("ss:stick"))
	end,
	on_punch = function(pos, node, puncher, pointed_thing)
		mt_dig_node(pos, puncher)
...
-- WOOD FRAME
--[[
core.register_node("ss:wall_wood_half_frame", {
...
This way, the campfire nodes will exist within the Luanti 'registered_nodes' and
...
This way, these bag nodes will exist within the Luanti 'registered_nodes' and
'registered_items' tables for other mod functions to access their basic properties
like description, drawtype, groups, walkable, etc. 
...
-- on land
core.override_item("ss:itemdrop_bag", {
	drawtype = "plantlike",
...
-- underwater
core.override_item("ss:itemdrop_bag_in_water", {
...
core.override_item("ss:itemdrop_box", {
	tiles = {
...
local snow_groups = core.registered_items["default:snow"].groups
...
core.register_entity("ss:ammo_snowball", {
	initial_properties = {
		physical = false,
		visual = "sprite",
...
for i, flower_name in ipairs(nodeNames) do
	core.override_item("ss:flower_" .. flower_name, {
		drawtype = "plantlike",
		waving = 1,
...
for i, node_name in ipairs({"water_source", "water_flowing", "river_water_source", "river_water_flowing"}) do
	core.override_item("default:" .. node_name, { 
		move_resistance = 2,
	})
end
...
for i, node_name in ipairs(LARGER_PLANTLIFE_NODE_NAMES) do
	core.override_item(node_name, { buildable_to = false })
end
...
-- or small gappy nodes. if node CAN be placed, ensure to reduce the player's
-- inventory weight.
...
for node_name in pairs(core.registered_nodes) do
	...
	if DONOT_OVERRIDE_NODES[node_name] then
		...
		core.override_item(node_name, { after_place_node = function(pos, player, item, pointed_thing)
			debug(flag1, "\nafter_place_node() node_overrides.lua")
			--debug(flag12, "  pos: " .. core.pos_to_string(pos))
...
-- by overriding the 'after_destruct' function. Note: in the default Luanti game,
