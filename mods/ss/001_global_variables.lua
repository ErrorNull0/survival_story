...
-- ##### of 200_global_tables.lua with data. All tables are indexed by the item name
...
-- solid nodes that are not a full meter height, but cannot be dug up by node placement
...
-- items that have limited usage and impact player stats when used or consumed
ss.CONSUMABLE_ITEMS = {
    -- FOOD
    ["ss:apple"] = {
        cooldown = {"ingest", 4.0},
        hunger = { direction = "up", amount = 10, is_immediate = false },
        thirst = { direction = "up", amount = 4, is_immediate = false },
        immunity = { direction = "up", amount = 0.5, is_immediate = false }
    },
    ["ss:apple_dried"] = {
        cooldown = {"ingest", 3.0},
        hunger = { direction = "up", amount = 10, is_immediate = false }
    ...
    ["ss:blueberries"] = {
        cooldown = {"ingest", 2.0},
        hunger = { direction = "up", amount = 2, is_immediate = false },
        immunity = { direction = "up", amount = 0.75, is_immediate = false }
    },
    ...
    ["ss:mushroom_brown"] = {
        cooldown = {"ingest", 3.0},
        hunger = { direction = "up", amount = 5, is_immediate = false }
    },
    ["ss:mushroom_brown_dried"] = {
        cooldown = {"ingest", 2.0},
        hunger = { direction = "up", amount = 3, is_immediate = false }
    },
    ["ss:mushroom_red"] = {
        cooldown = {"ingest", 3.0},
        hunger = { direction = "up", amount = 5, is_immediate = false },
        immunity = { direction = "down", amount = 5, is_immediate = false }
    },
    ["ss:cactus"] = {
        cooldown = {"ingest", 3.0},
        health = { direction = "down", amount = 1, is_immediate = false },
        hunger = { direction = "up", amount = 10, is_immediate = false },
        thirst = { direction = "up", amount = 5, is_immediate = false },
        immunity = { direction = "down", amount = 10, is_immediate = false }
    },
    ["ss:cactus_dried"] = {
        cooldown = {"ingest", 3.0},
        hunger = { direction = "up", amount = 6, is_immediate = false },
        immunity = { direction = "down", amount = 4, is_immediate = false }
    },
    ["ss:mushroom_red_dried"] = {
        cooldown = {"ingest", 2.0},
        hunger = { direction = "up", amount = 3, is_immediate = false },
        immunity = { direction = "down", amount = 2, is_immediate = false }
    },
    ["ss:meat_raw_beef"] = {
        cooldown = {"ingest", 5.0},
        hunger = { direction = "up", amount = 20, is_immediate = false },
        immunity = { direction = "down", amount = 20, is_immediate = false }
    },
    ["ss:meat_raw_mutton"] = {
        cooldown = {"ingest", 5.0},
        hunger = { direction = "up", amount = 20, is_immediate = false },
        immunity = { direction = "down", amount = 25, is_immediate = false }
    },
    ["ss:meat_raw_pork"] = {
        cooldown = {"ingest", 5.0},
        hunger = { direction = "up", amount = 20, is_immediate = false },
        immunity = { direction = "down", amount = 30, is_immediate = false }
    },
    ...
    ["ss:cup_wood_water_murky"] = {
        cooldown = {"ingest", 3.0},
        thirst = { direction = "up", amount = 20, is_immediate = false },
        immunity = { direction = "down", amount = 10, is_immediate = false }
    },
    ["ss:cup_wood_water_boiled"] = {
        cooldown = {"ingest", 3.0},
        thirst = { direction = "up", amount = 20, is_immediate = false },
        immunity = { direction = "down", amount = 1, is_immediate = false }
    },
    ["ss:bowl_wood"] = {
        cooldown = {"action", 2.0}
    },
    ...
    ["ss:bandages_basic"] = {
        cooldown = {"action", 4.0},
        health = { direction = "up", amount = 3, is_immediate = false }
    },
    ["ss:bandages_medical"] = {
        cooldown = {"action", 4.0},
        health = { direction = "up", amount = 10, is_immediate = false }
    },
    ["ss:pain_pills"] = {
        cooldown = {"ingest", 3.0},
        health = { direction = "up", amount = 30, is_immediate = false },
        thirst = { direction = "down", amount = 10, is_immediate = false },
    },
    ["ss:health_shot"] = {
        cooldown = {"action", 3.0},
        health = { direction = "up", amount = 30, is_immediate = true }
    },
    ["ss:first_aid_kit"] = {
        cooldown = {"action", 5.0},
        health = { direction = "up", amount = 50, is_immediate = false }
    },
    ["ss:splint"] = {
        cooldown = {"action", 5.0},
        health = { direction = "up", amount = 2, is_immediate = false }
    },
}
...
to ss.notify() --]]
ss.NOTIFICATIONS = {
    --slot_invalid = "Wrong item for that slot.",
    slot_invalid = "Item cannot be used there.",
    inv_weight_max = "inventory weight overloaded",
    inv_weight_max_dropped = "item dropped - iventory weight overloaded",
    inv_space_full = "no inventory space",
    inv_space_full_dropped = "item dropped - no inventory space",
    storage_match_failed = "no matching items to transfer",
    storage_weight_max = "storage weight overloaded",
    pickup_liquid_fail = "item will spill if moved there",
    not_empty_slot = "cannot swap - find empty slot"
}
...
<player_name> = {
    ...
    immunity_depletion_factor_health = 0.5,
    avatar_texture_base = ""
    avatar_texture_skin = "",
    fs = ""
}
...
-- the main table that hold buffs to be applied toward a player stat. this includes
-- 'inifite' buffs that loop perpetually or temporary buffs from consuming items, etc.
ss.stat_buffs = {}
...
ss.ENABLE_PERMANENT_STAT_BUFFS = true


--[[ Holds the minetest.after() job handles relating to player stat buffs and hud
updates, that are still active and waiting to execute their code. the handles are
used to immediately cancel the minetest.after() executions when needed. Example:
{
    [player_name] = {
        [buff_id_12] = <handle_object>,
        [buff_id_27] = <handle_object>,
        [buff_id_995] = <handle_object>,
        [message_box_1] = <handle_object>
    },
    [player_name] = {
        [buff_id_51] = <handle_object>,
        [buff_id_92] = <handle_object>,
        [buff_id_890] = <handle_object>,
        [message_box_2] = <handle_object>
    }
}
--]]
ss.job_handles = {}
...
-- indexed by player name. example: item_use_cooldowns[player_name][cooldown_type] 
ss.item_use_cooldowns = {}


--[[ Holds the the sound handle returned by function minetest.sound_play(). Indexed
...
Table is indexed by a pos hash via minetest.hash_node_position() based on player's
