...
local function reset_bundle_grid(player, p_data)
    ...
    if dropped_item_count > 0 then
        notify(player, dropped_item_count .. " items dropped to the ground", NOTIFY_DURATION, "message_box_2")
    end
...
local function reset_bundle_slot(player)
    ...
    if player_inv:is_empty("bundle_slot") then
        ...
        if player_inv:room_for_item("main", bundle_item) then
            ...
            mt_add_item(player:get_pos(), bundle_item)
            notify(player, "Item bundle dropped to the ground", NOTIFY_DURATION, "message_box_2")
        end
...
local function cancel_view_bundle(player, p_data)
    debug(flag8, "  cancel_view_bundle() bundle.lua")
        ...
        if player_inv:room_for_item("main", bundle_item) then
            player_inv:add_item("main", bundle_item)
            debug(flag8, "    item bundle returned to main inv")
        else
...
local function get_fs_tab_bundle(player_inv, p_data, player_meta, item_name)
    ...
    local fs_elements = table_concat({
        ...
        "tabheader[0,0;inv_tabs;Main,Bundle,Skills,Settings,?,*;2;true;true]",
        ...
        if from_list == "main" then
            ...
            if to_list == "main" then
                ...
                if SPILLABLE_ITEM_NAMES[item_name] then
                    ...
                    if to_index > 8 then
                        ...
                        notify(player, NOTIFICATIONS.pickup_liquid_fail, NOTIFY_DURATION, "message_box_3")
                        quantity_allowed = 0
                    else
            ...
            elseif to_list == "bundle_grid" then
                ...
                if p_data.bundle_item_count > 0 then
                    ...
                    if to_index > 1 then
                        ...
                        if item_name == p_data.bundle_item_name then
                            ...
                            if to_item:is_empty() then
                                ...
                                notify(player, "Slot must be empty", NOTIFY_DURATION, "message_box_3")
                                quantity_allowed = 0
                            end
                        else
                            debug(flag3, "  item is NOT same type as primary item. not allowed")
                            notify(player, "Only same item types can be bundled", NOTIFY_DURATION, "message_box_3")
                            quantity_allowed = 0
                        end

                    else
                        debug(flag3, "  moving to primary slot. not allowed.")
                        notify(player, "Primary slot already occupied", NOTIFY_DURATION, "message_box_3")
                        quantity_allowed = 0
                    end
                    ...
                    if SPILLABLE_ITEM_NAMES[item_name] then
                        ...
                        notify(player, "Spillable items cannot be bundled", NOTIFY_DURATION, "message_box_3")
                        quantity_allowed = 0
                    elseif item_name == "ss:item_bundle" then
                        debug(flag3, "  item bundles not allowed.")
                        notify(player, "Item bundles cannot be bundled further", NOTIFY_DURATION, "message_box_3")
                        quantity_allowed = 0
                    else
            ...
            elseif to_list == "bundle_slot" then
                ...
                if item_name == "ss:item_bundle" then
                    ...
                    quantity_allowed = 0
                    notify(player, "Item must be a bundle", NOTIFY_DURATION, "message_box_3")
                end
                ...
                if from_index > 1 then
                    ...
                    if to_index > 1 then
                        ...
                        if to_item:is_empty() then
                            ...
                            notify(player, "Primary bundle slot cannot become empty", NOTIFY_DURATION, "message_box_3")
                            quantity_allowed = 0
                        else
            ...
            elseif to_list == "main" then
                ...
                if from_index > 1 then
                    ...
                    if p_data.bundle_item_count > 1 then
                        ...
                        notify(player, "Remove other bundle items first.", NOTIFY_DURATION, "message_box_3")
                        quantity_allowed = 0
                    else
                        debug(flag3, "  allowed")
                    end
    ...
    elseif action == "take" then
        ...
        if listname == "bundle_grid" then
            ...
            if from_index > 1 then
                ...
                if p_data.bundle_item_count > 1 then
                    debug(flag3, "  Cannot remove primary bundle item while other bundle items remain.")
                    notify(player, "Cannot drop primary item while others remain", NOTIFY_DURATION, "message_box_3")
                    quantity_allowed = 0
                else
...
minetest.register_on_player_inventory_action(function(player, action, inventory, inventory_info)
    ...
    if action == "move" then
        ...
        if from_list == "main" then
            ...
            elseif to_list == "bundle_grid" then
                ...
                mt_show_formspec(player:get_player_name(), "ss:ui_bundle", formspec)
    ...
    elseif action == "take" then
        ...
        play_item_sound("item_move", {item_name = item:get_name(), player_name = player_name})
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
    ...
    if fields.inv_tabs == "2" then
        ...
        debug(flag1, "  did not click on main BUNDLE tab")
        ...
        elseif p_data.active_tab ~= "bundle" then
            ...
            elseif bundle_status == "view_bundle" then
                ...
                notify(player, "Bundle closed", NOTIFY_DURATION, "message_box_2")
                mt_after(0.25, function()
                    play_item_sound("bundle_close", {player_name = player_name})
                end)
            else
                debug(flag1, "  was updating or creating a bundle. resetting item slots..")
                reset_bundle_grid(player, p_data)
                notify(player, "Bundle items placed into inventory", NOTIFY_DURATION, "message_box_2")
                mt_after(0.25, function()
                    play_item_sound("bundle_cancel", {player_name = player_name})
                end)
            end
            debug(flag1, "register_on_player_receive_fields() END " .. mt_get_gametime())
            return
        ...
        elseif fields.bundle_open then
            ...
            local player_inv = player:get_inventory()
            if player_inv:is_empty("bundle_slot") then
                notify(player, "Slot requires an item bundle", NOTIFY_DURATION, "message_box_3")
            else
                play_item_sound("bundle_open", {player_name = player_name})
                p_data.bundle_status = "view_bundle"
                local formspec = get_fs_tab_bundle(player:get_inventory(), p_data, player:get_meta())
                mt_show_formspec(player:get_player_name(), "ss:ui_bundle", formspec)
            end
        ...
        elseif fields.bundle_save then
            ...
            if bundle_item_count > 1 then
                ...
                -- no space in inve. drop new bundle item to the ground.
                else
                    mt_add_item(player:get_pos(), bundle_item)

                    -- reduce inventory weight based on the bundle item's weight
                    set_stat(player, player_meta, "weight", "down", bundle_weight)
                    ...
                    notify(player, "No inventory space", NOTIFY_DURATION, "message_box_3")
                    notify(player, "New bundle item dropped to ground.", NOTIFY_DURATION, "message_box_2")
                end
                ...
                play_item_sound("button", {player_name = player_name})
                notify(player, "Bundle needs 2 or more items", NOTIFY_DURATION, "message_box_3")
            end
