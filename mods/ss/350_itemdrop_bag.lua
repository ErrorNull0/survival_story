...
local mt_get_node_drops = core.get_node_drops
...
local function transform_to_bag(self)
	debug(flag7, "\ntransform_to_bag()")
	if not self then
        print("### core.after(): self object invalid. quitting. itemdrop_bag.lua > transform_to_bag()")
        return
    end
...
-- override all dropped item entities to turn into an itemdrop bag after a certain
-- amount of time 'TRANSFORM_THRESHOLD_TIME' has elapsed
...
-- set newly defined 'item' as new __builtin:item, with the old one as fallback table
...
settings from the main Luanti settings page.
