...
-- time in seconds between checking hotbar if wield item has changed
...
minetest.register_entity("ss:wield_item", {
    initial_properties = {
        --visual = "sprite",
...
local function remove_wield_item(player)
    ...
    for object in minetest.objects_inside_radius(player:get_pos(), 10) do
...
local function monitor_wield_items(player)
    debug(flag1, "\nmonitor_wield_items(" .. player:get_player_name() .. ")")
    local player_name = player:get_player_name()
    local wielded_item = player:get_wielded_item()
    local item_name = wielded_item:get_name()
    local item_name_prev = wield_item_name_prev[player_name] or ""
    debug(flag1, "  item_name_prev: " .. item_name_prev)
    ...
    if item_name == item_name_prev then
        ...
        debug(flag1, "  child_entities: " .. dump(child_entities))
        ...
        if item_name == "" then
            ...
            notify(player, item_description, 1, "message_box_1")
            if wield_item_entity then
                debug(flag1, "    updating item wield entity...")
                wield_item_entity:set_properties({wield_item = item_name})
                local rotation
                if custom_rotations[item_name] then
                    debug(flag1, "  applying custom rotation...")
                    rotation = custom_rotations[item_name]
                else
                    debug(flag1, "  default rotation...")
    ...
    mt_after(refresh_rate, monitor_wield_items, player)
end
...
minetest.register_on_joinplayer(function(player)
    ...
    mt_after(1, function()
        ...
        if wield_item then
            ...
            for object in minetest.objects_inside_radius(player:get_pos(), 1) do
                if object:is_player() then
...
minetest.register_on_shutdown(function()
    debug(flag4, "register_on_shutdown() wield_item.lua")
    debug(flag4, "register_on_shutdown() END")
