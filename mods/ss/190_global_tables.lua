...
time to perform actions based on the "minetest.registered_" tables, like initialize
...
for craftitem_name, item_def in pairs(minetest.registered_craftitems) do
...
for node_name, node_def in pairs(minetest.registered_nodes) do
	...
	elseif minetest.get_item_group(node_name, "water") > 0 then
		ss.NODE_NAMES_WATER[node_name] = true
		debug(flag4, "    added to NODE_NAMES_WATER")

	elseif minetest.get_item_group(node_name, "lava") > 0 then
		ss.NODE_NAMES_LAVA[node_name] = true
		debug(flag4, "    added to NODE_NAMES_LAVA")

	elseif minetest.get_item_group(node_name, "stair") > 0 then
	...
	elseif minetest.get_item_group(node_name, "slab") > 0 then
...
local flag2 = false
local file_path = minetest.get_modpath("ss") .. "/campfire_data.txt"
