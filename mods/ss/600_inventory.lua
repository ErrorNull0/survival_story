...
function ss.get_fs_tabs_left()
    return { "tabheader[0,0;inv_tabs;Main,Bundle,Skills,Settings,?,*;1;true;true]" }
end
...
function ss.get_fs_item_info(player, item)
    ...
    if not (player and item) then
        ...
        local survival_tip_number = math_random(#SURVIVAL_TIPS)
        local survival_tip_data = SURVIVAL_TIPS[survival_tip_number]
        local survival_tip_title = survival_tip_data[1]
        local survival_tip_text = survival_tip_data[2]

        return {

            --table_concat({"hypertext[", 5.9, ",0.1;5,1;game_version_info;",
                --"<style color=#777777 size=14>Survival Story v0.0.1</style>", "]"}),
            ...
            --[[ survival tips will be moved to its own dedicated tab and listed in a contest view
            table_concat({"hypertext[6.0,0.5;7,1;survival_tip_title;",
                "<style color=#AAAAAA size=26><b>Random Survival Tip #", survival_tip_number, "</b></style>", "]"}),

            table_concat({"hypertext[6.0,1.3;8.7,1.3;survival_tip_text;",
                "<style color=#AAAAAA size=18><b>", survival_tip_title, "</b></style> ",
                "<style color=#777777 size=18>", survival_tip_text, "</style>", "]"}),
            --]]
        }
...
local function get_fs_craft_item_icon(item_name)
    ...
    elseif new_inv_size < INV_SIZE_MIN then
        ...
        if amount < 0 then
            ...
            if items_were_dropped then
                notify(player, "Items dropped due to reduced inventory space.", NOTIFY_DURATION, "message_box_3")
...
local flag12 = false
-- if 'delta' exists, then the weight max is adjusted by the value of delta.
-- if delta does not exist, the weight max is recalculated by how many slots
-- exist in the player's 'main' inventory location.
--- @param player ObjectRef the player object
...
function ss.update_inventory_weight_max(player, fs)
    ...
    player_meta:set_float("weight_max", new_max_weight)
...
local function show_item_info(player, item, p_data, action)
    ...
    -- when the player takes an item from a different inv location and places onto item info slot
    elseif action == "put" then
        if item_name == prev_item_name then
            debug(flag19, "    current and prev item is the SAME")
            if item_name == "ss:item_bundle" then
                debug(flag19, "    this is an item bundle. always refresh..")
                fs.center.item_info = get_fs_item_info(player, item)
                refresh_inventory_ui = true
            ...
            debug(flag19, "    curr and prev items are DIFFERENT. updating item info...")
            fs.center.item_info = get_fs_item_info(player, item)
            fs.right.craft_button = {}
            fs.right.ingredients_box = {}
            refresh_inventory_ui = true
        end
...
-- this is a global wrapper function so that cooking_station.lua can call this while
-- the inventory-related functions within can remain local within inventory.lua
function ss.refresh_fs_crafting(p_data, fs, player_name, player_inv)
    local prev_recipe_id = p_data.prev_recipe_id
    local recipe = RECIPES[prev_recipe_id]
    if recipe then
        local itemstack = ItemStack(recipe.icon)
        fs.right.ingredients_box = ss.get_fs_ingred_box(player_name, itemstack, player_inv, prev_recipe_id)
        local crafting_count = ss.get_craftable_count(player_inv, prev_recipe_id)
        fs.right.craft_button = ss.get_fs_craft_button(prev_recipe_id, crafting_count)
    end
...
minetest.register_on_joinplayer(function(player)
	...
	elseif player_status == 1 then
        ...
        --p_data.fs = minetest.deserialize(player_meta:get_string("fs"))
        local saved_fs = minetest.deserialize(player_meta:get_string("fs"))
...
minetest.register_allow_player_inventory_action(function(player, action, inventory, inventory_info)
    ...
    if p_data.formspec_mode ~= "main_formspec" then
		...
		debug(flag2, "register_allow_player_inventory_action() END " .. minetest.get_gametime())
		return
    ...
    if action == "move" then
        ...
        elseif to_list == "clothing_slot_eyes" then
            ...
            if CLOTHING_EYES[item_name] then
                ...
                notify(player, "Not an eye accessory", NOTIFY_DURATION, "message_box_3")
                quantity_allowed = 0
            end
        ...
        elseif to_list == "clothing_slot_neck" then
            ...
            if CLOTHING_NECK[item_name] then
                ...
                notify(player, "Not a neck accessory", NOTIFY_DURATION, "message_box_3")
                quantity_allowed = 0
            end
        ...
        elseif to_list == "clothing_slot_chest" then
            ...
            if CLOTHING_CHEST[item_name] then
                ...
                notify(player, "Not clothing for upper body", NOTIFY_DURATION, "message_box_3")
                quantity_allowed = 0
            end
        ...
        elseif to_list == "clothing_slot_hands" then
            ...
            if CLOTHING_HANDS[item_name] then
                ...
                notify(player, "Not wearable on hands", NOTIFY_DURATION, "message_box_3")
                quantity_allowed = 0
            end
        ...
        elseif to_list == "clothing_slot_legs" then
            ...
            if CLOTHING_LEGS[item_name] then
                ...
                notify(player, "Not clothing for lower body", NOTIFY_DURATION, "message_box_3")
                quantity_allowed = 0
            end
        ...
        elseif to_list == "clothing_slot_feet" then
            ...
            if CLOTHING_FEET[item_name] then
                ...
                notify(player, "Not proper foot support", NOTIFY_DURATION, "message_box_3")
                quantity_allowed = 0
            end
        ...
        elseif to_list == "armor_slot_head" then
            ...
            if ARMOR_HEAD[item_name] then
                ...
                notify(player, "Not head armor", NOTIFY_DURATION, "message_box_3")
                quantity_allowed = 0
            end
        ...
        elseif to_list == "armor_slot_face" then
            ...
            if ARMOR_FACE[item_name] then
                ...
                notify(player, "Not face protection", NOTIFY_DURATION, "message_box_3")
                quantity_allowed = 0
            end
        ...
        elseif to_list == "armor_slot_chest" then
            ...
            if ARMOR_CHEST[item_name] then
                ...
                notify(player, "Not chest armor", NOTIFY_DURATION, "message_box_3")
                quantity_allowed = 0
            end
        ...
        elseif to_list == "armor_slot_arms" then
            ...
            if ARMOR_ARMS[item_name] then
                ...
                notify(player, "Not arm protection", NOTIFY_DURATION, "message_box_3")
                quantity_allowed = 0
            end
        ...
        elseif to_list == "armor_slot_legs" then
            ...
            if ARMOR_LEGS[item_name] then
                ...
                notify(player, "Not leg protection", NOTIFY_DURATION, "message_box_3")
                quantity_allowed = 0
            end
        ...
        elseif to_list == "armor_slot_feet" then
            ...
            if ARMOR_FEET[item_name] then
                ...
                notify(player, "Not feet protection", NOTIFY_DURATION, "message_box_3")
                quantity_allowed = 0
            end
            ...
            if BAG_NODE_NAMES_ALL[item_name] then
                ...
                if to_item:is_empty() then
                    ...
                    if item_count > 1 then
                        ...
                        notify(player, "only 1 bag per slot allowed", NOTIFY_DURATION, "message_box_2")
                        quantity_allowed = 1
                    end
                ...
                notify(player, "Only bags can be equipped", NOTIFY_DURATION, "message_box_3")
                quantity_allowed = 0
            end
        ...
        elseif to_list == "main" then
            ...
            notify(player, NOTIFICATIONS.slot_invalid, NOTIFY_DURATION, "message_box_2")
            quantity_allowed = 0
        end
...
minetest.register_on_player_inventory_action(function(player, action, inventory, inventory_info)
    ...
    if p_data.formspec_mode ~= "main_formspec" then
		...
		debug(flag8, "register_allow_player_inventory_action() end " .. minetest.get_gametime())
		return
	elseif p_data.active_tab ~= "main" then
    ...
    if action == "move" then
        ...
        if from_list == "main" then
            ...
            if to_list == "bag_slots" then
                ...
                local formspec = build_fs(fs)
                player:set_inventory_formspec(formspec)
                mt_show_formspec(player:get_player_name(), "ss:ui_main", formspec)
    ...
    elseif action == "take" then
        ...
        elseif listname == "main" then
            ...
            if p_data.formspec_mode == "main_formspec" then
                ...
                -- this only happens upon join new game when no recipe item was clicked
                -- on yet and is still NIL
                if recipe_id == "" then
                    debug(flag8, "  No prior recipe item clicked. Skipped refresh of ingred box and craft button.")
                else
                    debug(flag8, "  Loading recipe_id and refreshing ingred_box and craft button...")
                    local recipe = RECIPES[recipe_id]
                    local crafting_count = ss.get_craftable_count(inventory, recipe_id)
                    fs.right.ingredients_box = get_fs_ingred_box(player_name, ItemStack(recipe.icon), inventory, recipe_id)
                    fs.right.craft_button = ss.get_fs_craft_button(recipe_id, crafting_count)
                end
                debug(flag8, "  Refreshing crafting grid...")
                fs.right.craft_grid = get_fs_crafting_grid(player_name, inventory, p_data.recipe_category)
    ...
    debug(flag8, "register_on_player_inventory_action() end " .. minetest.get_gametime())
end)
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
    ...
    if fields.inv_tabs == "1" then
        ...
        debug(flag3, "register_on_player_receive_fields() end " .. minetest.get_gametime())
        return
        ...
        elseif p_data.active_tab ~= "main" then
            ...
            debug(flag3, "register_allow_player_inventory_action() END " .. minetest.get_gametime())
            return
    ...
    elseif field_type == "enRecipeX" then
        ...
        if recipe_id == prev_recipe_id then
            ...
            if #fs.right.craft_button < 0 then
                debug(flag3, "  craft button currently visible. no action.")
            else
                debug(flag3, "  craft button currently not visible. enabling button...")
            ...
            debug(flag3, "  refreshed craft item icon and ingredients box.")
    ...
    elseif field_type == "enCraftbutton" then
        ...
        if inv_too_heavy then
            notify(player, NOTIFICATIONS.inv_weight_max_dropped, NOTIFY_DURATION, "message_box_3")
        elseif inv_space_full then
            notify(player, NOTIFICATIONS.inv_space_full_dropped, NOTIFY_DURATION, "message_box_3")
        end
        ...
        ss.set_stat(player, player_meta, "experience", "up", experience_gained)

        -- cycle through the recipe ingredients to see if player inv contains enough
        -- items to craft the recipe item again.
        ...
        if craftable_again then
            ...
            debug(flag3, "  checking if item can still be crafted with alt group items..")
            local crafting_count = ss.get_craftable_count(player_inv, recipe_id)

            if crafting_count > 0 then
                debug(flag3, "  YES. alt group ingredients allow " .. crafting_count .. " more crafting.")
                fs.right.craft_button = ss.get_fs_craft_button(recipe_id, crafting_count)
            else
                debug(flag3, "  No alt group ingredients avail.")
                fs.right.craft_button = {}
            end
        end
