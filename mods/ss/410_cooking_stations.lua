...
local mt_get_player_by_name = minetest.get_player_by_name
...
local mt_check_for_falling = minetest.check_for_falling
...
local mt_add_particlespawner = minetest.add_particlespawner
local mt_delete_particlespawner = minetest.delete_particlespawner
...
local ITEM_COOK_PATH = {
	...
	-- items that will douse the fire. must be listed here to allow cooking worm-up
	-- mechanic, but table element value is unused and set to empty string.
	["default:snow"] = "",
	["ss:snow_pile"] = "",
}
...
-- currently used to store cooking xp owed to players who went offline before the item
-- finished cooking.
...
-- indexed by player name. stores the minetest.after job produced from turning on the
...
local file_path = minetest.get_modpath("ss") .. "/cook_nodes.txt"
...
local flag24 = false
-- cancels the looping minetest.after() calls that resfreshes and displays the campfire
...
local function update_weight_data(player, player_meta, p_data, fs, weight, direction)
	-- update vertical statbar weight HUD
	set_stat(player, player_meta, "weight", direction, weight)
	...
	formspec = get_fs_campfire(player, player:get_player_name(), pos)
	mt_show_formspec(player:get_player_name(), "ss:ui_campfire", formspec)
end
...
local function reduce_ingred_fuel_slots(player, node_meta, node_inv, pos, slot_to_reduce)
	...
	if slot_item:is_empty() then
		...
		if player then
			notify(player, slot_to_reduce ..  " item dropped to ground.", NOTIFY_DURATION, "message_box_2")
		else
...
local function apply_wear_tool(pos, node_meta, node_inv, tool_item, tool_type, duration)
	...
	if condition > 0 then
		...
		play_item_sound("item_break", {item_name = item_name, pos = pos})
...
local function cook_ingredient(item, slot_index, node_inv, pos, duration)
	...
	if heat_progress < COOK_THRESHOLD then
		...
		play_item_sound("campfire_cooked", {pos = pos})
		...
		if player then
			set_stat(player, player:get_meta(), "experience", "up", xp_gain_cooking)
		...
		elseif HEAT_DESTRUCT_CONTAINERS[item_name] then
			...
			mt_after(0.25, play_item_sound, "item_break", {item_name = item_name, pos = pos})
...
local function heat_node(pos, location, duration)
	...
	-- cook_time = 10 -- for testing purposes
...
local function get_fuel_item_count(player, node_meta, item)
	...
	if item_count_that_fits < item_count then
		if item_count_that_fits > 0 then
			...
			notify(player, "Only " .. item_count_that_fits .. " max could be added to slot.", NOTIFY_DURATION, "message_box_2")
		else
			debug(flag15, "  itemstack too heavy to fit")
			notify(player, "Item exceeds fuel weight max.", NOTIFY_DURATION, "message_box_3")
		end
	else
...
local function stop_campfire(pos, pos_key, node_meta)
	...
	play_item_sound("campfire_stop" , {pos = pos})
...
local function campfire_after_place_node(pos, player, item, pointed_thing)
	...
	if node_name == "air" then
		...
		notify(player,"area below not supportive", 3, "message_box_2")
...
local function campfire_burn_loop(pos, elapsed)
	...
	if burn_time_extra > 0 then
		...
		play_item_sound("campfire_start", {pos = pos})
...
local function campfire_on_rightclick(pos, node, clicker, itemstack, pointed_thing)
	...
	play_item_sound("campfire_open", {pos = pos})
...
local function campfire_allow_metadata_inventory_move(pos, from_list, from_index, to_list, to_index, count, player)
	...
	if from_list == "ingredients" then
		if to_list == "ingredients" then
			...
			if to_item:is_empty() then
				...
				if to_item_name == item_name then
					...
					move_count = 0
					notify(player, "Only 1 allowed in ingredient slot.", NOTIFY_DURATION, "message_box_3")
				else
		...
		elseif to_list == "campfire_stand" then
			...
			if campfire_status == "on" then
				...
				move_count = 0
				notify(player, "Cannot place while campfire is on.", NOTIFY_DURATION, "message_box_3")
			else
				if CAMPFIRE_STAND_NAMES[item_name] then
					...
					if to_item:is_empty() then
						...
						if to_item_name == item_name then
							...
							debug(flag13, "  prevent the move action as it would increase the stack count")
							move_count = 0
							notify(player, "Only 1 allowed in campfire stand slot.", NOTIFY_DURATION, "message_box_3")
						else
							debug(flag13, "  item in slot and item being moved are DIFFERENT")
							move_count = 1
						end
					...
					move_count = 0
					notify(player, "Only campfire stands allowed in stand slot.", NOTIFY_DURATION, "message_box_3")
				end
			end
		...
		elseif to_list == "campfire_grill" then
			...
			if campfire_status == "on" then
				...
				if CAMPFIRE_GRILL_NAMES[item_name] then
					...
					move_count = 0
					notify(player, "Only campfire grills allowed in grill slot.", NOTIFY_DURATION, "message_box_3")
				end
			end
		...
		elseif to_list == "fire_starter" then
			...
			if FIRE_STARTER_NAMES[item_name] then
				...
				move_count = 0
				notify(player, "Only fire starters allowed in fire starter slot.", NOTIFY_DURATION, "message_box_3")
			end
		...
		elseif to_list == "fuel" then
			...
			if ITEM_BURN_TIMES[item_name] > 0 then
				...
				move_count = 0
				notify(player, "Item not burnable as fuel.", NOTIFY_DURATION, "message_box_3")
			end
	...
	elseif from_list == "campfire_stand" then
		...
		if campfire_status == "on" then
			debug(flag13, "  campfire is ON")
			move_count = 0
			notify(player, "Cannot remove stand while campfire is on.", NOTIFY_DURATION, "message_box_3")
		else
			if to_list == "ingredients" then
			...
			elseif to_list == "campfire_grill" then
				...
				debug(flag13, "  campfire stand never allowed to campfire grill slot")
					move_count = 0
					notify(player, "Only campfire grills allowed.", NOTIFY_DURATION, "message_box_3")
	...
	elseif from_list == "campfire_grill" then
		...
		if campfire_status == "on" then
			debug(flag13, "  campfire is ON")
			move_count = 0
			notify(player, "Cannot remove grill while campfire is on.", NOTIFY_DURATION, "message_box_3")
		else
			if to_list == "ingredients" then
			...
			elseif to_list == "campfire_stand" then
				...
				debug(flag13, "  campfire grill never allowed into campfire stand slot")
					move_count = 0
					notify(player, "Only stands allowed in campfire stand slot.", NOTIFY_DURATION, "message_box_3")
	...
	elseif from_list == "fire_starter" then
		if to_list == "ingredients" then
			...
			if to_item:is_empty() then
				...
				if to_item_name == item_name then
					...
					debug(flag13, "  prevent the move action as it would increase the stack count")
					move_count = 0
					notify(player, "Only 1 allowed in ingredients slot.", NOTIFY_DURATION, "message_box_3")
				else
					debug(flag13, "  item in slot and item being moved are DIFFERENT")
					move_count = 1
					...
					if FIRE_STARTER_NAMES[to_item_name] then
						...
						debug(flag13, "  item is not a fire starter")
						notify(player, "Cannot swap with a non fire starter tool.", NOTIFY_DURATION, "message_box_3")
					end
				end
			end
		...
		elseif to_list == "campfire_grill" then
			...
			debug(flag13, "  fire starter never allowed into campfire grill slot")
				move_count = 0
				notify(player, "Only grills allowed in campfire grill slot.", NOTIFY_DURATION, "message_box_3")
	...
	elseif from_list == "fuel" then
		if to_list == "ingredients" then
			...
			local to_item = node_inv:get_stack("ingredients", to_index)
			if item_name == "ss:item_bundle" then
				notify(player, "Cannot cook items while bundled", NOTIFY_DURATION, "message_box_3")
				move_count = 0
			elseif to_item:is_empty() then
				debug(flag13, "  target slot is empty")
				if count > 1 then
					notify(player, "Only 1 item added", NOTIFY_DURATION, "message_box_2")
					move_count = 1
				end
			else
				local to_item_name = to_item:get_name()
				...
				if to_item_name == item_name then
					...
					if ITEM_BURN_TIMES[to_item_name] > 0 then
						...
						debug(flag13, "  item is not a campfire fuel")
						notify(player, "Cannot swap with a non-burnable item.", NOTIFY_DURATION, "message_box_3")
					end
				end
			end
				...
				if CAMPFIRE_GRILL_NAMES[item_name] then
					...
					if to_item:is_empty() then
						...
						if to_item_name == item_name then
							...
							debug(flag13, "  prevent the move action as it would increase the stack count")
							move_count = 0
							notify(player, "Only 1 allowed in campfire grill slot.", NOTIFY_DURATION, "message_box_3")
						else
							debug(flag13, "  item in slot and item being moved are DIFFERENT")
							move_count = 1
						end
			...
			if FIRE_STARTER_NAMES[item_name] then
				...
				if to_item:is_empty() then
					...
					if to_item_name == item_name then
						...
						debug(flag13, "  prevent the move action as it would increase the stack count")
						move_count = 0
						notify(player, "Only 1 allowed in fire starter slot.", NOTIFY_DURATION, "message_box_3")
					else
						debug(flag13, "  item in slot and item being moved are DIFFERENT")
						notify(player, "Only 1 item added", NOTIFY_DURATION, "message_box_2")
						move_count = 1
	...
	if listname == "ingredients" then
		...
		elseif to_item:is_empty() then
			...
			if to_item_name == item_name then
				...
				if item_count > 1 then
					notify(player, "Item count too large to swap.", NOTIFY_DURATION, "message_box_3")
					put_count = 0
				else
					if item_count > 1 then
						notify(player, "Only 1 item added", NOTIFY_DURATION, "message_box_2")
...
local function campfire_on_metadata_inventory_move(pos, from_list, from_index, to_list, to_index, count, player)
	...
	play_item_sound("item_move", {item_name = item_name, player_name = player:get_player_name()})
...
minetest.register_on_joinplayer(function(player)
	debug(flag1, "\nregister_on_joinplayer() CAMPFIRE")
	local player_meta = player:get_meta()
	...
	if player_status == 1 then
		...
		if cooking_xp_owed > 0 then
			...
			set_stat(player, player:get_meta(), "experience", "up", cooking_xp_owed)
...
minetest.register_on_player_receive_fields(function(player, formname, fields)
	...
	elseif fields.campfire_start then
		...
		if node_inv:is_empty("fire_starter") then
			...
			notify(player, "Fire starter tool is missing.", NOTIFY_DURATION, "message_box_3")
		...
		-- fire starter item is equipped
			...
			play_item_sound("item_use", {item_name = fire_starter_name, player = player})
			...
			if remaining_uses > 0 then
				...
				play_item_sound("item_break", {item_name = fire_starter_name, pos = pos})
				...
				if broken_items then
					...
					notify(player, "Item broke. Scraps dropped to ground.", 3, "message_box_2")
				else
			...
			if fire_starter_name == "ss:fire_drill" then
				...
				local stamina_loss = p_data.stamina_loss_fire_drill
				debug(flag2, "    stamina_loss: " .. stamina_loss)
				set_stat(player, player_meta, "stamina", "down", stamina_loss)

				if p_data.exhausted then
					debug(flag2, "    exhausted! extreme draining of thirst, hunger, sanity, and immunity")
					set_stat(player, player_meta, "thirst", "down", stamina_loss * p_data.thirst_loss_factor_stamina * p_data.exhaustion_factor)
					set_stat(player, player_meta, "hunger", "down", stamina_loss * p_data.hunger_loss_factor_stamina * p_data.exhaustion_factor)
					set_stat(player, player_meta, "sanity", "down", stamina_loss * p_data.sanity_loss_factor_stamina * p_data.exhaustion_factor)
					set_stat(player, player_meta, "immunity", "down", stamina_loss * p_data.immunity_loss_factor_exhaustion)
				else
					debug(flag2, "    not exhuasted. normal draining of thirst, hunger, and sanity")
					set_stat(player, player_meta, "thirst", "down", stamina_loss * p_data.thirst_loss_factor_stamina)
					set_stat(player, player_meta, "hunger", "down", stamina_loss * p_data.hunger_loss_factor_stamina)
					set_stat(player, player_meta, "sanity", "down", stamina_loss * p_data.sanity_loss_factor_stamina)
				end
				...
				if random_num > p_data.fire_drill_success_rate then
					...
					notify(player, "Failed to light fire drill. Try again.", NOTIFY_DURATION, "message_box_3")
			...
			elseif fire_starter_name == "ss:match_book" then
				...
				if random_num > p_data.match_book_success_rate then
					...
					notify(player, "Match failed to light. Try another.", NOTIFY_DURATION, "message_box_3")
					fire_start_success = false
				end
			...
			if fire_start_success then
				...
				for i, ingredient_item in ipairs(ingredient_slots) do
					debug(flag2, "    slot #" .. i)
					if ingredient_item:is_empty() then
						debug(flag2, "    empty slot")
					else
						local ingredient_item_name = ingredient_item:get_name()
						debug(flag2, "    ingredient item found: " .. ingredient_item_name)
						debug(flag2, "    campfire turned on while item in ingred slot")
						check_ingredient_status(ingredient_item, node_inv, i, player)
					end
				end

				debug(flag2, "    refresh campfire formspec to reflect 'on' state")
				node_meta:set_string("campfire_status", "on")

				play_item_sound("campfire_start", {pos = pos})

				debug(flag2, "    activating node timer in 3 seconds")
				local node_timer = mt_get_node_timer(pos)
				node_timer:start(FUEL_BURN_INTERVAL)

				debug(flag2, "    replace campfire node with ss:campfire_small_burning")
				mt_swap_node(pos, {name = "ss:campfire_small_burning"})

				debug(flag2, "    adding smoke effect above campfire")
	...
	elseif fields.campfire_rebuild then
		...
		if missing_ingredients then
			...
			notify(player, "Missing campfire ingredients.", NOTIFY_DURATION, "message_box_3")

		else
			debug(flag2, "  necessary ingredients avail. rebuilding campfire...")
			play_item_sound("item_use", {item_name = "ss:campfire_small_new", player = player})
			...
			set_stat(player, player_meta, "weight", "down", campfire_ingredients_weight)
			fs.center.weight = get_fs_weight(player)

			debug(flag2, "  refresh crafting pane and ingred box to reflect consumed items..")
			ss.refresh_fs_crafting(p_data, fs, player_name, player_inv)
            ...
            set_stat(player, player_meta, "experience", "up", experience_gained)
...
minetest.register_allow_player_inventory_action(function(player, action, inventory, inventory_info)
		...
		if to_list == "main" then
			...
			if SPILLABLE_ITEM_NAMES[item_name] then
                ...
                if to_index > 8 then
                    ...
                    notify(player, NOTIFICATIONS.pickup_liquid_fail, NOTIFY_DURATION, "message_box_2")
                    block_action = true
                end
            end

		else
			notify(player, "Unexpected target: " .. to_list, NOTIFY_DURATION, "message_box_3")
			block_action = true
		end
	...
	elseif action == "put" then
		...
		if not to_item:is_empty() then
			notify(player, NOTIFICATIONS.not_empty_slot, NOTIFY_DURATION, "message_box_3")
			block_action = true
		...
		elseif SPILLABLE_ITEM_NAMES[item_name] then
			...
			if to_index > 8 then
				...
				if exceeds_inv_weight_max(item, player_meta) then
					notify(player, NOTIFICATIONS.inv_weight_max, NOTIFY_DURATION, "message_box_3")
					block_action = true
				end
			end
...
minetest.register_on_player_inventory_action(function(player, action, inventory, inventory_info)
	...
	elseif action == "take" then
		...
		update_weight_data(player, player_meta, p_data, fs, weight, "down")
	...
	elseif action == "put" then
		...
		debug(flag7, "  weight: " .. weight)
		update_weight_data(player, player_meta, p_data, fs, weight, "up")
...
minetest.register_lbm({
    name = "ss:reactivate_campfire_smoke",
    nodenames = {"ss:campfire_small_burning"},
    run_at_every_load = true,
    action = function(pos, node)
		debug(flag27, "\nregister_lbm() reactivate_campfire_smoke")
		local node_id = mt_hash_node_position(pos)
		local particle_id = particle_ids[node_id]
		if particle_id then
