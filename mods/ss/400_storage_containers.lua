...
local flag13 = false
local function update_weight_dislay(player, player_meta, p_data, fs, weight, direction)
	debug(flag13, "  update_weight_dislay()")
	debug(flag13, "    weight: " .. weight)
	debug(flag13, "    direction: " .. direction)
	...
	if SPILLABLE_ITEM_NAMES[item_name] then
		...
		if total_weight > weight_max then
			...
			if item_count > 1 then
				...
				notify(player, NOTIFICATIONS.storage_weight_max, NOTIFY_DURATION, "message_box_3")
				quantity_allowed = 0
			end
		end
	end
...
local function storage_on_metadata_inventory_take(pos, listname, index, stack, player)
	...
	local node_meta = mt_get_meta(pos)
	play_item_sound("item_move", {item_name = stack:get_name(), player_name = player:get_player_name()})
...
minetest.register_on_player_inventory_action(function(player, action, inventory, inventory_info)
	...
	elseif action == "take" then
		...
		if listname == "main" then
			...
			print ("  Unexpected listname: " .. listname)
		end

		update_weight_dislay(player, player_meta, p_data, fs, weight, "down")
	...
	elseif action == "put" then
		...
		debug(flag8, "  weight: " .. weight)
		update_weight_dislay(player, player_meta, p_data, fs, weight, "up")
